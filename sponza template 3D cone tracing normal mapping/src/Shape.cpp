...
void Shape::loadMesh(const string &meshName, string *mtlpath, unsigned char *(loadimage)(char const *, int *, int *, int *, int))
{
	...
	else if (shapes.size())
	{
		...
		binormBuf = new std::vector<float>[shapes.size()];
	...
	for (int i = 0; i < objMaterials.size(); i++)
		...
		if (objMaterials[i].normal_texname.size() > 0)
			{
			char filepath[1000];
			int width, height, channels;
			string filename = objMaterials[i].normal_texname;
			int subdir = filename.rfind("\\");
			if (subdir > 0)
				filename = filename.substr(subdir + 1);
			string str = *mtlpath + filename;
			...
			if (data)
				{
				glGenTextures(1, &normalmapsIDs[i]);
				glActiveTexture(GL_TEXTURE1);
				glBindTexture(GL_TEXTURE_2D, normalmapsIDs[i]);
				glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
				glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
				glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
				glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
...
void Shape::init()
{
...

	{
		...
		if (binormBuf[i].empty())
		{
			binormBufID[i] = 0;
		}
		else
		{
			glGenBuffers(1, &binormBufID[i]);
			glBindBuffer(GL_ARRAY_BUFFER, binormBufID[i]);
			glBufferData(GL_ARRAY_BUFFER, binormBuf[i].size() * sizeof(float), binormBuf[i].data(), GL_STATIC_DRAW);
		}
...

	{
		...
		if (!use_extern_texures)
		{
			...
			if (textureindex >= 0)
			{
				...
				glActiveTexture(GL_TEXTURE1);
				glBindTexture(GL_TEXTURE_2D, normalmapsIDs[textureindex]);
			}
		}
		// Draw
		glDrawElements(GL_TRIANGLES, (int)eleBuf[i].size(), GL_UNSIGNED_INT, (const void *)0);
