...
bool bmpfont::init()
	...
	unsigned char* data = stbi_load("bmpfontclear.png", &width, &height, &channels, 4);
...
void main()
{
	...
	pos.xy *= offset_pos.z;//scaling
	pos.xy += offset_pos.xy;
	gl_Position = pos;
...
//-----------------------------------------------------------------------------
void bmpfont::draw(float x, float y, std::string text)
{
	...
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, textureID);
	vec3 offset_position = vec3(x, y, fontsize);
	int letters = text.size();
	//glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
	for (int i = 0; i < letters; i++)
	{		
		glUniform3fv(uniform_pos_offset, 1, &offset_position.x);
		fontvec3 offset_texposition = get_texturecoord_offset(text[i]);
